...
export default function WhatIfSimulator() {
  ...
  const runSimulation = async () => {
    ...
    try {
      const payload = changes.map(c => ({ productId: c.productId, newPrice: c.newPrice, newCost: c.newCost }));
        ...
        <CardContent className="space-y-4">
          ...
          {changes.length > 0 && (
            <div className="space-y-2">
              <div className="grid grid-cols-6 gap-2 text-[10px] font-medium text-gray-500 px-2">
              ...
              {changes.map((c, idx) => (
                <div key={idx} className="grid grid-cols-6 gap-2 items-center bg-gray-50 rounded-lg p-2" data-testid={`sim-row-${idx}`}>
          ...
          <div className="grid grid-cols-3 gap-4">
            <Card className="border-0 shadow-sm">
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold" style={{ color: theme.text }}>${results.totalCurrentProfit.toFixed(0)}</p>
              </CardContent>
            </Card>
            <Card className="border-0 shadow-sm">
              <CardContent className="p-4">
                <p className="text-xs text-gray-500">Projected Profit</p>
                <p className="text-2xl font-bold" style={{ color: theme.primary }}>${results.totalProjectedProfit.toFixed(0)}</p>
              </CardContent>
            </Card>
            <Card className="border-0 shadow-sm" style={{ background: results.netImpact >= 0 ? '#ECFDF5' : '#FEF2F2' }}>
              <CardContent className="p-4">
                <p className="text-xs text-gray-500">Net Impact</p>
                <div className="flex items-center gap-2">
                  {results.netImpact >= 0 ? <TrendingUp size={20} className="text-green-600" /> : <TrendingDown size={20} className="text-red-600" />}
                  <p className="text-2xl font-bold" style={{ color: results.netImpact >= 0 ? '#10B981' : '#EF4444' }}>
                    {results.netImpact >= 0 ? '+' : ''}${results.netImpact.toFixed(0)}
                  </p>
                </div>
              </CardContent>
            </Card>
                  ...
                  {results.simulations.map((sim, i) => (
                    <tr key={i} className="border-b hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-2 text-right">
                        ...
                        <span className="font-bold">${sim.projectedPrice}</span>
                      </td>
                      <td className="px-4 py-2 text-right">
                        <span className="text-gray-400">{sim.currentQty}</span>
                        <ArrowRight size={10} className="inline mx-1 text-gray-300" />
                        <span>{sim.projectedQty}</span>
                      </td>
                      <td className="px-4 py-2 text-right font-mono">${sim.currentProfit.toFixed(0)}</td>
                      <td className="px-4 py-2 text-right font-mono font-bold">${sim.projectedProfit.toFixed(0)}</td>
                      <td className="px-4 py-2 text-right">
                        <Badge style={{
                          background: sim.profitChange >= 0 ? '#ECFDF5' : '#FEF2F2',
                          color: sim.profitChange >= 0 ? '#10B981' : '#EF4444'
                        }}>
                          {sim.profitChange >= 0 ? '+' : ''}{sim.profitChangePct}%
                        </Badge>
                      </td>
                    </tr>
                  ))}

...
export function CFD() {
  ...
  useEffect(() => { const tick = () => v25API.cfdCurrent().then(r => setData(r.data || { cart: [] })); tick(); const id = setInterval(tick, 5000); return () => clearInterval(id); }, []);
  ...
  return (
    <div className="min-h-screen bg-black text-white p-8 -m-6" data-testid="cfd-page">
      <h1 className="text-5xl font-bold mb-8 flex items-center gap-3"><Monitor /> Welcome</h1>
      <div className="space-y-3">
        {(data.cart || []).map((it, i) => (
      ...
      {data.customer && <p className="mt-6 text-amber-400">Welcome back, {data.customer.name} ⭐</p>}
    </div>
  );
}

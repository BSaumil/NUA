...
import { X, Send, Clock, Users, Utensils, ChevronRight, Settings2, BellRing } from 'lucide-react';
import { tableCoursesAPI } from '../../services/api';
...
export const TableInfoDrawer = ({ open, onClose, table, states, courses, overdueColour, onRefresh, onOpenCourseSettings }) => {
  ...
  const seatTable = async () => {
    setBusy(true);
    try {
      await tableCoursesAPI.upsertState({ tableId: table.id, course: 'seated', partySize: table.capacity || 2 });
  ...
  return (
    <div
      className="fixed inset-y-0 right-0 z-50 w-full sm:w-96 bg-white shadow-2xl border-l flex flex-col"
      style={{ borderLeft: `4px solid ${state?.colour || '#e5e7eb'}` }}
    ...
    >
      <div className="p-4 border-b flex items-center justify-between" style={{ background: state?.colour || '#f9fafb' }}>
        <div className="text-white">
          <div className="text-xs uppercase tracking-widest opacity-80">Table</div>
        ...
        {state ? (
              ...
              <CardContent className="p-3 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-gray-500 flex items-center gap-1">
                    <Clock size={11} /> in course {courseMin}m {overdue && <span className="text-red-600 font-semibold">· OVERDUE</span>}
                  </div>
                </div>
          ...
          <Card data-testid="table-empty">
            <CardContent className="p-3 text-center text-sm text-gray-500">
              No live state — table isn&apos;t seated.
              <Button onClick={seatTable} disabled={busy} size="sm" className="mt-2 w-full" data-testid="seat-table">

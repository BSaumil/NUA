...
async def list_accounts(_: dict = Depends(get_user)):
    rows = await db.accounts.find({}, {"_id": 0}).sort("code", 1).to_list(500)
...
async def create_account(body: dict, user: dict = Depends(require_owner_or_manager)):
    ...
    acc = Account(**{**body, "code": code}).dict()
...
async def update_account(code: str, body: dict, user: dict = Depends(require_owner_or_manager)):
    ...
    upd = {k: v for k, v in body.items() if k not in ("code", "id")}
    await db.accounts.update_one({"code": code}, {"$set": upd})
    return await db.accounts.find_one({"code": code}, {"_id": 0})
...
async def delete_account(code: str, user: dict = Depends(require_owner_or_manager)):
    ...
    used = await db.journal_entries.count_documents({"lines.accountCode": code, "posted": True})
    if used:
        raise HTTPException(400, f"Account is used in {used} journal entries — deactivate instead")
    await db.accounts.delete_one({"code": code})
    return {"deleted": True}


@router.post("/seed")
async def seed(_: dict = Depends(require_owner_or_manager)):
...
async def get_journal(jid: str, _: dict = Depends(get_user)):
...
async def report_trial_balance(as_of: Optional[str] = None, _: dict = Depends(get_user)):
    return await svc.trial_balance(as_of=as_of)
...
async def report_profit_loss(
    ...
    d = _default_period()
    return await svc.profit_and_loss(from_date or d["from"], to_date or d["to"])


@router.get("/reports/balance-sheet")
async def report_balance_sheet(as_of: Optional[str] = None, _: dict = Depends(get_user)):
...
async def report_cash_flow(
    ...
    d = _default_period()
    return await svc.cash_flow(from_date or d["from"], to_date or d["to"])
...
async def report_general_ledger(
    ...
    return await svc.general_ledger(code, from_date=from_date, to_date=to_date)
...
async def report_budget_vs_actual(
    ...
    if budget_id:
        ...
        budget = await db.budgets.find_one({}, {"_id": 0}, sort=[("createdAt", -1)])
    actuals = await svc._account_totals_between(from_d, to_d)
    accounts = await svc._account_map()
...
async def pay_bill(bid: str, body: dict, user: dict = Depends(require_owner_or_manager)):
    ...
    try:
        je = await svc.auto_post_bill_payment(payment)
    except ValueError as e:
        raise HTTPException(400, str(e))
    ...
    await db.bills.update_one({"id": bid}, {"$set": {"paidAmount": round(new_paid, 2), "status": new_status}})
    return {"payment": payment, "newStatus": new_status, "paidAmount": round(new_paid, 2)}


@router.delete("/bills/{bid}")
async def delete_bill(bid: str, _: dict = Depends(require_owner_or_manager)):
    bill = await db.bills.find_one({"id": bid}, {"_id": 0})
    if not bill:
        raise HTTPException(404, "Bill not found")
    ...
    if bill.get("journalEntryId"):
        try:
            await svc.reverse_entry(bill["journalEntryId"], memo="Bill deleted")
        except ValueError:
            pass
...
# ═════════════════════════════════════════════════════════════════════════
@router.get("/invoices")
async def list_invoices(status: Optional[str] = None, customer_id: Optional[str] = None, _: dict = Depends(get_user)):
    q: Dict[str, Any] = {}
    if status: q["status"] = status
...
async def receive_invoice(iid: str, body: dict, user: dict = Depends(require_owner_or_manager)):
    ...
    try:
        je = await svc.auto_post_invoice_receipt(receipt)
    except ValueError as e:
        raise HTTPException(400, str(e))
    ...
    await db.ar_invoices.update_one({"id": iid}, {"$set": {"paidAmount": round(new_paid, 2), "status": new_status}})
    return {"receipt": receipt, "newStatus": new_status, "paidAmount": round(new_paid, 2)}


@router.delete("/invoices/{iid}")
async def delete_invoice(iid: str, _: dict = Depends(require_owner_or_manager)):
    inv = await db.ar_invoices.find_one({"id": iid}, {"_id": 0})
    if not inv:
        raise HTTPException(404, "Invoice not found")
    ...
    if inv.get("journalEntryId"):
        try:
            await svc.reverse_entry(inv["journalEntryId"], memo="Invoice deleted")
        except ValueError:
            pass
...
# ═════════════════════════════════════════════════════════════════════════
@router.get("/deposits")
async def list_deposits(status: Optional[str] = None, _: dict = Depends(get_user)):
    q: Dict[str, Any] = {}
    if status: q["status"] = status
...
async def create_deposit(body: dict, user: dict = Depends(require_owner_or_manager)):
    dep = CustomerDeposit(**body).dict()
    try:
        je = await svc.auto_post_customer_deposit(dep)
...
async def apply_deposit(did: str, body: dict, user: dict = Depends(require_owner_or_manager)):
    ...
    try:
        je = await svc.auto_post_deposit_applied(dep, {"id": txn_id})
    except ValueError as e:
        raise HTTPException(400, str(e))
    await db.customer_deposits.update_one(
        {"id": did}, {"$set": {"status": "applied", "appliedTransactionId": txn_id,
...
async def bank_statement(code: str, unmatched_only: bool = False, _: dict = Depends(get_user)):
    ...
    for r in rows:
        ...
        if sd:
            ...
            cand = await db.journal_entries.find(
                {"date": {"$gte": lo, "$lte": hi}, "lines.accountCode": code},
                {"_id": 0},
            ).to_list(50)
            for je in cand:
                for l in je["lines"]:
...
async def bank_import(body: dict, _: dict = Depends(require_owner_or_manager)):
    ...
    inserted = 0
    for l in lines_in:
        doc = BankStatementLine(**{**l, "accountCode": account_code}).dict()
        if doc.get("externalId"):
            exists = await db.bank_statement_lines.find_one({"externalId": doc["externalId"], "accountCode": account_code})
            if exists:
                continue
...
async def bank_match(line_id: str, journal_id: str, _: dict = Depends(require_owner_or_manager)):
...
async def bank_ignore(line_id: str, _: dict = Depends(require_owner_or_manager)):
...
# ═════════════════════════════════════════════════════════════════════════
@router.get("/budgets")
async def list_budgets(_: dict = Depends(get_user)):
    rows = await db.budgets.find({}, {"_id": 0}).sort("createdAt", -1).to_list(50)
    return safe_parse_list(rows, Budget, where="budgets")


@router.post("/budgets")
async def create_budget(body: dict, _: dict = Depends(require_owner_or_manager)):
    b = Budget(**body).dict()
...
# ═════════════════════════════════════════════════════════════════════════
@router.get("/kpis")
async def kpis(_: dict = Depends(get_user)):
    today = datetime.now(timezone.utc).date()
    fy_start = f"{today.year if today.month >= 7 else today.year - 1}-07-01"
    to_d = today.isoformat()

    pnl = await svc.profit_and_loss(fy_start, to_d)
    bs = await svc.balance_sheet(as_of=to_d)
    cash = await svc.cash_flow(fy_start, to_d)

    # AR/AP outstanding
    ar_open = await db.ar_invoices.find({"status": {"$in": ["unpaid", "partial", "overdue"]}}, {"_id": 0}).to_list(1000)
    ap_open = await db.bills.find({"status": {"$in": ["unpaid", "partial", "overdue"]}}, {"_id": 0}).to_list(1000)

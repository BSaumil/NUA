...
async def member_signup(req: MemberSignup, response: Response):
    ...
    member = {
        ...
        "referralCode": f"NUA-{str(uuid.uuid4())[:6].upper()}",
...
async def get_share_link(member_id: str):
    ...
    return {
        ...
        "shareText": f"Join me at NUA! Sign up with my code {ref_code} and get 15% off your first meal.",
        "referralCode": ref_code,
        "socialLinks": {
            "facebook": f"https://www.facebook.com/sharer/sharer.php?u=nua.com{base}?ref={ref_code}",
            "twitter": f"https://twitter.com/intent/tweet?text=Join%20me%20at%20NUA!%20Use%20code%20{ref_code}&url=nua.com{base}",
            "whatsapp": f"https://wa.me/?text=Join%20me%20at%20NUA!%20Sign%20up%20with%20code%20{ref_code}%20and%20get%2015%25%20off!",
        }
    }

...
async def list_channel_menu(channel: str):
    ...
    overrides_list = await db.channel_menus.find({"channel": channel}, {"_id": 0}).to_list(2000)
...
async def patch_row(channel: str, body: ChannelMenuPatch, _: dict = Depends(require_owner_or_manager)):
    ...
    res = await db.channel_menus.update_one(
        {"channel": channel, "productId": body.productId},
...
async def ai_discount_slow_movers(channel: str, body: dict, _: dict = Depends(require_owner_or_manager)):
    ...
    all_products = await db.products.find({}, {"_id": 0, "id": 1, "name": 1, "price": 1}).to_list(2000)

...
def test_wallet_tier_matches_the_real_loyalty_tiers_ladder(client, owner_headers):
    ...
    # against whatever the seed defaults happen to be.
    _run(db.loyalty_tiers.delete_many({}))
    _run(db.loyalty_tiers.insert_many([
        {"id": "t-bronze", "name": "Bronze", "minPoints": 0, "multiplier": 1.0, "discountPercent": 0, "perks": []},
        {"id": "t-gold", "name": "Gold", "minPoints": 300, "multiplier": 1.5, "discountPercent": 5, "perks": []},
    ]))

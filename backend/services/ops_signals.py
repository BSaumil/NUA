...
async def _recently_emitted(source: str) -> bool:
...
async def check_server_errors(*, lookback_minutes: int = LOOKBACK_MINUTES,
                                threshold: int = SERVER_ERROR_THRESHOLD) -> Dict[str, Any]:
    cutoff = datetime.now(timezone.utc) - timedelta(minutes=lookback_minutes)
    rows = await db.error_log.find(
        {"at": {"$gte": cutoff}}, {"_id": 0, "path": 1, "error": 1},
    ).to_list(500)
    count = len(rows)
...
async def check_client_errors(*, lookback_minutes: int = LOOKBACK_MINUTES,
                                threshold: int = CLIENT_ERROR_THRESHOLD) -> Dict[str, Any]:
    cutoff = datetime.now(timezone.utc) - timedelta(minutes=lookback_minutes)
    rows = await db.client_error_log.find(
        {"at": {"$gte": cutoff}}, {"_id": 0, "message": 1, "url": 1},
    ).to_list(500)
    count = len(rows)
    result: Dict[str, Any] = {"triggered": count >= threshold, "count": count,
                               "threshold": threshold, "windowMinutes": lookback_minutes}
...
async def scan_and_emit() -> Dict[str, Any]:
    """Called from the hourly Ash scheduler loop, same as predictive_signals.
    Checks both sources; emits ops.error_spike for whichever crossed its
    threshold and isn't already inside its dedupe window."""
    server = await check_server_errors()
    server["emitted"] = False
    if server["triggered"] and not await _recently_emitted("server"):
    ...
    client = await check_client_errors()
    client["emitted"] = False
    if client["triggered"] and not await _recently_emitted("client"):
        await rules_engine.emit_event("ops.error_spike", {
